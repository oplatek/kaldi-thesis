...
Namely we will describe only {\it delta + delta-delta}, \ac{LDA} and \ac{MLLT},
...
The {\it delta + delta-delta}\/ feature transformation 
...
The {\it delta + delta-delta}\/ method adds features to \ac{MFCC} feature vector, which approximates 
first and second derivative of \ac{MFCC} features.
See Figure~\ref{fig:delta} for typical setup of {\it delta + delta-delta}\/ for \ac{MFCC},
where from 13 \ac{MFCC} features 13 {\it delta}\/ and another 13 {\it delta-delta}\/ features are extracted.
...
\subsubsection*{\acl{LDA}}
\ac{LDA} is used as linear classifier for acoustic features. \todo{citace}
It transforms the acoustic features to another feature space with reduced dimension.
\todo{it sound like it does not add features only transforms}

\subsubsection*{\acl{MLLT}}
\ac{MLLT}\cite{psutka2007benefit} clusters and 
transforms full covariance matrices for Gaussian distributions used in \ac{HMM}s. 
Using the full covariance matrices instead of the diagonal one 
improves the \ac{WER} and the sharing of the covariance matrices makes the technique reasonable efficient.
