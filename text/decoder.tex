...
As a result, the speech decoding can be performed as user speaks and the ASR output is obtained with minimal latency.
...
Next Section~\ref{sec:pyext} describes, \term{PyOnlineLatgenRecogniser}, a Python extension of C++ \term{OnlineLatgenRecogniser}.
...
In addition, we subclassed \term{LatticeFasterDecoder} and reorganized its~original batch interface so it supports on-line interface.
Such interface almost eliminates latency of a recogniser since almost all the decoding can be performed as user speaks.

First, we present the new on-line interface of \term{OnlineLatgenRecogniser} and in next subsections we introduce its components.
...
The interface is greatly influenced by the decoder interface and the preprocessing of the utterance is completely hidden for the user of \term{OnlineLatgenRecogniser}.
The \term{AudioIn} is the single method which is not related to the decoders functionality. 
...
Please note that the~recogniser's latency is mainly determined by the~time spent in the~\term{GetLattice} function since the whole loop is processed as user speaks.
...
We are able to decode the utterance as user speaks.
...
The decoding is performed on request by calling the \term{Decode} method with parameter (int max\_frames) which limits the number of decoded frames.
...
The speed of the Viterbi search is highly predictable and can be controlled by decoding parameters so \term{max\_frames} allows the whole recogniser stay responsive as illustrated in Listing~\ref{snippets/rec_usage.cc}. 

On the other hand, \ac{ASR} output is extracted by original methods of \term{LatticeFasterDecoder}:
\begin{itemize}
...
The state-level lattice, which is returned from the \term{GetRawLattice} method, can be understand as lattice on triphone level.
...
The \term{beam} parameter affects the speed of all utterances, whereas the \term{max-active-states} parameter plays its role for utterances with lot of uncertainty in beam search due to noises.
In fact, the \term{max-active-states} is a threshold for worst case scenarios.
The \term{lattice-beam} beam influence speed of lattice extraction. 
...
The resulting acoustic features are used with an \ac{AM} in \term{OnlLatticeFasterDecoder} to obtain likelihood of each state explored by Viterbi search.
...
When a decoder is asked to performed forward decoding it needs to estimate likelihood for the states which should be explored, so it request the \term{DecodableInterface}.
We implemented on-line version of \term{DecodableInterface} which let the decoder ask for likelihoods for new acoustic features frame by frame.
...
\begin{itemize}
    ...
    \item The \term{OnlDecodableDiagGmmScaled} queries the \ac{AM} for the probability for acoustic features $a$ and given state.
...
The on-line implementation \term{OnlDecodableDiagGmmScaled} of \term{DecodableInterface} easily handles if no audio data are available.
If \term{OnlDecodableDiagGmmScaled} has no acoustic features precomputed, it asks the feature transformation to compute new features.
If the feature transformation can not provide the features, so it returns default empty value. 
The empty value signals to \term{OnlDecodableDiagGmmScaled} that no features are not available at the moment. 
Finally, \term{OnlDecodableDiagGmmScaled} produces also default empty value and the decoder's method \term{Decode(int max\_frames)} returns zero indicating that no frames were decoded.
