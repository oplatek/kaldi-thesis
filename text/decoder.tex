...
\begin{itemize}
    ...
    \item \term{GetBestPath} finally return just one-best bath.
\end{itemize}
The state-level lattice returned from \term{GetRawLattice} method can be understand as lattice on triphone level.
Single word hypothesis is typically represented as multiple state-level hypotheses since the words are aligned differently, which means that the same words were pronounced at different time according the state-level hypotheses.




    furthprocessed states  pruning and later marking the last frame of the utterance as final.
...
The pre-processing and the decoder are bridged by the \term{decodable} interface as illustrated in Figure~\ref{fig:online_pipeline}.
...
In addition, we developed a~Python extension, \term{PyOnlineLatgenRecogniser}, exporting the~\term{OnlineLatgenRecogniser} C++ interface.
...
The output of the \term{OnlineLatgenRecogniser} is represented as OpenFST lattices and is input is raw binary data.
