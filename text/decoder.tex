...
\label{sub:dec}
We did not implement any new functionality \term{OnlLatticeFasterDecoder}, but we only reorganised the code of base class \term{LatticeFasterDecoder}.
...
In \term{OnlineLatgenRecogniser},  we control the beam search by the~following functions: 
\begin{itemize}
...
The \term{beam} parameter affects the speed of all utterances, whereas the \term{max-active-states} parameter plays its role for utterances with uncertainty in beam search due to noises.
...
The empty value signals to \term{OnlDecodableDiagGmmScaled} that no features are available at the moment. 
