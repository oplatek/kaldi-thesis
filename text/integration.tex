...
If \ac{VAD} detects start of speech in the~input audio stream, it passes the speech signal to \ac{ASR} component and the~\term{rec\_in} method is called. 
...
In Kaldi implementation of \term{rec\_in}, the audio is decoded using beam search as user speaks, i.e., the method \term{rec\_in} gradually adds the new audio in \term{PyOnlineLatgenRecogniser} buffer and immediately decodes it.\footnote{If the \ac{ASR} component is busy with decoding the audio just waits in \ac{VAD} buffer instead of in \term{PyOnlineLatgenRecogniser}'s buffer.}
...
In~the Kaldi real-time settings, latency of the~\ac{ASR} unit depends mostly on~the time spent in \term{hyp\_out} method.
...
As the script's results, the \term{HCLG} build script produces follwing files which are required for decoding with \term{PyOnlineLatgenRecogniser}:
\begin{itemize}
    \item \todo{Fill the files}
\end{itemize}
