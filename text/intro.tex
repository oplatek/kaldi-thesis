...
% We would like to achieve at least as accurate decoding with Kaldi acoustic models as we have achieved with \ac{HTK} models. In this thesis we will set up and evaluate following experiments using Kaldi:
...
The speed of~the decoders will be expressed in terms of~\ac{RTF}\footnote{\ac{RTF} is metric for comparing speed of~decoders. It is defined as $\ac{RTF} = \frac{P}{D}$, where $P$ is time of~the processing the audio by a~decoder on input with length $D$.}. The accuracy will be expressed in terms of~\ac{WER}.
