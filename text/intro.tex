...
The Alex dialog system has used the \ac{HTK} toolkit\cite{young94htk} and OpenJulius\cite{lee2009julius} lattice decoder in order to train acoustic models respectively to decode lattices in real time. 
...
We modify Kaldi \term{LatticeFasterDecoder} decoder in order to allow incremental speech recognition.
...
The process of incremental speech recognition is represented by single instance of \term{OnlineLatgenRecogniser} class,
...
We interface \term{OnlineLatgenRecogniser} via its Python wrapper since the dialogue system is written in Python.
The \term{PyOnlineLatgenRecogniser} is a thin wrapper which efficiently exposes the speech recognition interfaces.
