...
The task of acoustic modeling is to estimate the parameters $\theta$ of a model so the probability $P(a \mid w ; \theta)$ is as accurate as possible, which is described in Section~\ref{sub:am}. 
...
\todo{Describe the ASR pipeline - describe figure 2.1}
...
% Note that speech parametrisation transforms the acoustic signal based on few numerical parameters, which are constant for all recording conditions during training and decoding.
...
Some transformations take into account context of several preceding (left context) and consecutive frames (right context).
...
\todo{Some transforms are estimated on all acoustic data, some transformations are trained on data selected per speaker.}
...
\begin{itemize}
    \item \acf{LDA}\cite{gopinath1998maximum}
    \item \acf{HLDA}\cite{gales1999semi}
    \item \acf{MLLT} also known as \acf{STC}\cite{gopinath1998maximum}
    \item \acf{ET}\cite{povey2011exponential}
    \item \acf{CMVN}\cite{molau2003feature}
\end{itemize}
...
The combination of \ac{LDA} and \ac{MLLT} applies the feature transformation in two steps: \ac{LDA} reduces the feature dimension and \ac{MLLT} rotates the feature space so the classes after \ac{LDA} has more diagonal covariance in two steps.\cite{gopinath1998maximum}
...
\begin{figure}[!htp]
    ...
    \caption{Markov phone model for three words. Transition probabilities within
...
of~a~discrete-time series with an unknown state. \todo{Chapter 8, \cite{huang2001spoken}}.
...
\ml{Gaussian HMM}
\todo{In speech recognition, a multivariate Gaussian distribution is used to model observation probabilities of \ac{HMM} states.} 
...
\todo{The Kaldi as well as the~\ac{HTK} toolkit uses \acl{EM} algorithms to train \ac{HMM} \acl{AM}.}
...
We focus on Viterbi training because it is used in Kaldi toolkit.
...
The word sequence "\term{are few born new york}" should be rewarded with a lower probability from \ac{LM} than the alternative with similar phonetic representation "\term{are you from new york}".
...
other application for example in a dialogue system prefer
