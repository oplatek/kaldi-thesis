...
The infrequent word sequences in training data are estimated with a low probability in a \ac{LM}. 
On the other hand, the frequent sequences are assigned with a high probability.
For example, the word sequence "\term{are few born new york}" should be rewarded with a lower probability from a~\ac{LM} than the alternative with similar phonetic representation "\term{are you from new york}".
