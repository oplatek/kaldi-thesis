...
\begin{figure}[!htp]
    \begin{center}
        ...
        The arrows at \ac{HMM} states illustrates that every observation of acoustic features can generated according statistical distribution. 
        Note that if speaker says {\it two} a \ac{HMM} model with well trained parameters should output higher probability
        for \ac{HMM} representing {\it two}. For longer speaker sequences e.g. {\it two one one two ...} the \acp{HMM} are connected over the 
$\epsilon$ transitions and search is used for selecting the most probable sequence.}
        \label{fig:hmm_alt} 
    \end{center}
\end{figure}
...
where cost of one corresponds to zero probability $C(1)~P(0)$ 
and cost of infinity corresponds to one probability $C(\infty)~P(1)$.
...
In many applications formats than one-best hypothesis output format
provide better results for further processing, because other hypothesis
...
N-best list is extension to one-best hypothesis, where except the most probable word sequence,
the second, third, ...,n-th most probable hypothesis is written to output. 
Usually, also the probability measure is reported with each word sequence.
Notice on that the probabilities of 3-best list on Figure~\ref{fig:nbest}
does not sum one, because n-best list typically omits some hypothesis 
...
\todo{how nbest list are extracted cite -do not describe}
\todo{TODO cite lattice generation Kaldi and reference section skip/the process in decsription}
\todo{figure lattice}
