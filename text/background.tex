...
\begin{enumerate}
    ...
    \item The~discrete signal is transformed into {\it frequency domain} by \ac{DFT}.
    \item \todo{following four items are taken from Wikipedia and should be polished and cited. Read it from Psutka}
    \item Map the powers of the frequency spectrum obtained above onto the mel scale\footnote{https://en.wikipedia.org/wiki/Mel_scale}, using triangular overlapping windows.
...
% TODO describe sampling https://en.wikipedia.org/wiki/Sampling_(signal_processing)
\end{enumerate}
