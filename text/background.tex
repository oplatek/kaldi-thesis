...
% \term{Supervised learning} is the~machine learning task of inferring a function from labeled training data.\cite{mohri2012foundations}. 
...
% The inferred function is often called \term{model}. 
% 
% \term{Unsupervised learning} can be thought of as finding patterns in the input data beyond what would be considered 
...
\small{\begin{enumerate}
    ...
    \item The delta and delta-delta coefficients are computed from the current and previous static features. See Figure~\ref{fig:delta}.
\end{enumerate}
...
In Section~\ref{sec:transform} we describe in more detail the transforms, which we use in our training scripts.
...
It distinguishes the~pronunciation for \term{earned} and \term{ear}
...
Hidden Markov Models have two type of parameters \term{transition probabilities among states}
and \term{probabilistic distribution for generating observation in given state}.
...
In Figure~\ref{fig:hmm} the transcription \term{how do you do} 
...
The best alignment \todo{T} results from maximising Equation~\ref{eq:vit_align} for $ 1 < i < N$.
...
Secondly, transition probabilities are estimated from the relative frequencies, Equation~\ref{eq:freq}. 

\begin{equation}\label{eq:freq}
   todo eq 4 from buthpitiya2012parallel
\end{equation}

\todo{Add equations 4+ from buthpitiya2012parallel!}
...
\begin{itemize}
    \item \term{Fixed beam} guarantees memory footprint and fast decoding.
    \item \term{One-best hypothesis comparison} effectively discards 
...
With the toolkit are distributed two decoders \term{HVite} and \term{HDecode},
...
The trained models are used in one of \ac{HTK} decoders e.g. \term{HVite} for decoding
transcriptions, which can be evaluated using \term{HResults}.
...
The \term{HVite} decoder uses token passing algorithm and Viterbi criterion.
...
The \term{HDecode} has very similar license condition but can be only used for research purposes.\footnote{You
...
% and the~dialog system Alex were waiting for the lattice output of dead process, which should had been passed through sockets.
...
Formally, a \term{semiring} $(\mathcal{K}, plus, krat, nula, jedna)$ is an algebraic structure on set $\mathcal{K}$ with operations $plus$ and $krat$.
The binary operations multiplication $plus$ and addition $krat$ have identity element $nula$ respectively $jedna$. 
The $(\mathcal{K}, plus)$ forms commutative monoid and $(\mathcal{K}, krat)$ forms just a monoid.
The multiplication is left and right distributive over addition.
Moreover, multiplication by $zero$ annihilates any member of $\mathcal{K}$ to $zero$. 
Table~\ref{fig:semiring} shows useful semirings in OpenFST.

\todo{translate table and krat, plus, nula, jedna to latex}

\begin{table}[!htp]\label{tab:semiring}
\begin{tabular}{lrrrrr}
\hline
Name & $\mathcal{K}$ & $plus$ & $krat$ & $nula$ & $jedna$ \\ 
\hline
Probability  &  $\mathcal{R}_{+}$  &  +  &  *  &  0  &  1  \\
% Log &    &  -log(e<sup>-x</sup> + e<sup>-y</sup>)  &  +  &  &infin;  &  0  \\
% Tropical &  [-&infin;, &infin;]  &  min  &  +  &  &infin;  &  0  \\
% \hline
\end{tabular}
\caption{Semirings used in speech recognition.}
\end{table}
