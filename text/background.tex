...
The acoustic modeling has only partial information available for training \ac{AM} parameters $\theta$ because the corresponding textual transcription is time-unaligned.
...
of~a~discrete-time series with an unknown state. \cite{huang2001spoken}. % huang2001spoken chapter 8
...
We create \ac{HMM} $t'$, where each state represents one monophone.\footnote{The state-of-the-art \acp{AM} uses triphones, but the training procedure is the same as for monophones which are easier to describe.} 
The monophones are extracted from transcription $t$ using pronunciation dictionary.
In Figure~\ref{fig:hmm_words} the transcription \term{how do you do} was the~expanded \ac{HMM} model representing monophone sequence.
...
We detail the Viterbi training since it is used in the~Kaldi toolkit for acoustic model training and also in~a~very similar algorithm used for Viterbi decoding.
...
It returns one if $o^r_t$ is \todo{associated} with mixture component $m$ of state $j$ and is zero otherwise.
...
The discriminative training uses its objective function and posterior probability to discriminate -- boost differences high probable hypotheses and low probable ones.
The discriminative training is typically initialised by acoustic generative model from Baum-Welsch or Viterbi training.
The likelihood from the generative model is boosted according an objective function.
In training scripts described in Chapter~\ref{cha:train} following objective functions and accordingly named discriminative training methods are used:
\begin{itemize}
...
The infrequent word sequences in training data are assign with low probability in a \ac{LM}. 
On the other hand, the frequent sequences are assigned with high probability.
The word sequence "\term{are few born new york}" should be rewarded with a lower probability from a~\ac{LM} than the alternative with similar phonetic representation "\term{are you from new york}".

Note that in our settings the \ac{AM} already limits the possible triphone sequences to sequences present in lexicon --- the words in training data.
...
\ml{LM smoothing}
We call the \term{smoothing} technique estimating the probability of unseen n-grams $t$ 
...
but the decoding is more complicated for \ac{ASR}.
...
Imagine we have a \ac{LM} order 1 of only two words - \term{one} and \term{two} each uniformly distributed\footnote{If a \ac{LM} of order 1 assigns to every word equal probability, we say it has order 0}.
We want to decode any possible sequence of words \term{one}, \term{two}, so we add additional $\epsilon$ transition at the end of words \acp{HMM}. 
...
\begin{itemize}
    ...
    \item \term{One-best hypothesis comparison} effectively discards most of the improbably hypothesis if the one-best hypothesis is significantly better than alternatives and keeps lot of alternatives if one-best hypothesis is weak.
...
\begin{verbatim}
    ...
    0.1 bey how are you
\end{verbatim}
\caption{Example of 3-best list output with posterior probability for each path. N-best in Kaldi can be easily extracted from lattices. Corresponding example lattice is in Figure~\ref{fig:toy_lat}.}
    \label{fig:nbest} 
    \end{center}
\end{figure}
...
\begin{figure}[!htp]
    ...
    \caption{Word posterior lattice. All outgoing arcs for each node sum to 1.0. Common parts of hypotheses are effectively represented.}
    \label{fig:toy_lat} 
    \end{center}
\end{figure}

A lattice is a convenient type of \ac{ASR} output.  
It effectively represent the alternative hypotheses by sharing their common parts.
Example of word lattice in~Figure~\ref{fig:toy_lat} show word posterior lattice.
...
It is useful to capture how the quality of each hypothesis in contrast to its alternatives or even in absolute measure. 
...
pair $decoded(a), t$ typically using edit operations \term{substitution, deletion, insertion} as described in~\ref{eq:edit_dist}.
...
\term{HVite} can be used only with unigram or bigram \ac{LM}.
The \term{HDecode} decoder handles also a trigram \ac{LM}.
...
Only bigram \ac{LM} can be used with \term{HVite}.
...
Similarly Kaldi uses binaries which load its input from files and typically store results again to files.
...
The scripts are located in \term{utils} and \term{steps} directories and are used in many training script recipes for different corpus data.
...
Decoding is performed using a final result of training, so called \term{decoding graph} $HCLG$. 
...
\ml{Semiring}
Most of the operation used in speech recognition operates on paths in the decoding graph.
Path is a sequence of edges which has weight and an input and an output label.
Based on the weight type and weight path operations we distinguish several semirigns. 
