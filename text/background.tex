...
For each window \ac{MFCC} or \ac{PLP} efficiently computes statistics with reduced dimension. 
...
The \ac{HTK} toolkit in {\bf E-step} finds the most probable distribution which map acoustic observation to \ac{HMM} states using \todo{MLE fill few words and cite}. 
...
Only in special cases of speaker adaptive discriminative training is also used extended Baum-Welsch algorithm\cite{TODO_POVEY}.
...
The C++ Kaldi library is based {\it OpenFST}\cite{OpenFST_TODO} library and 
...
Czech and English training corpus \todo{lrec2013}\cite{TODO_LREC2014}.
...
The \ac{FST} framework provides well studied graph operations\cite{TODO_Mohri},
...
As stated in \cite{TODO_Mohri} and \cite{TODO_Kaldi} the operations can be effectively used
for speech decoding. 
...
Note that proper introduction to \ac{FST} framework for speech recognition we recommend\cite{TODO_mohri}.
