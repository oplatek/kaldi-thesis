...
Speech parametrization extracts from speech audio signal distinctive acoustic features for speech.
...
The \ac{MFCC} transformations is applied on sampled and quantized audio signal.
The overlapping windows of several audio samples, Figure~\ref{fig:mfcc_window},
 is used to compute for \ac{MFCC} statistics.

\begin{figure}[!htp]
    \begin{center}
    \input{images/mfcc_window}
    \caption{\ac{PLP} or \ac{MFCC} features are computed every 10ms seconds in 50ms windows.
    Audio length is $(frames-1)*shift + win\_len = 110ms$}
    \label{fig:mfcc_window} 
    \end{center}
\end{figure}

The \ac{MFCC} feature transformation reduce 
\todo{describe MFCC}
\todo{Draw mfcc}


\small{\begin{enumerate}
    \item The~signal is transformed into {\it frequency domain}\/ by~\ac{DFT} in overlapping frames.
    \item The frequency spectrum obtained in previous step is transformed onto the mel scale, using triangular overlapping filters.
